...
21. **Rework requirements file structure** (use PLAN MODE — user reminder). Today the requirements tree is "modules nested inside modules" (submodules). The user wants organization by **pure container files/folders that requirement modules live inside** — i.e. a containing entity that groups modules, rather than module-in-module nesting. Rethink the `modules` self-parent model vs a new grouping level. Non-trivial data-model + tree-UI change — brainstorm → spec → plan before touching code.
