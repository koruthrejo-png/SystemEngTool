...
> **Status as of 2026-07-17.** **Done: 1-10, 13, 15-31** (all but the three below).
> **Open: 11** (admin area — item 13 now puts a "You"/"People" section in the Settings
> modal that could migrate there, alongside B1's type-colour editor),
> **12** (nav notification/help/profile icons — the ⚙ Settings gear shipped with item 29
> B1; the rest do not exist),
> **14** (export PDF from architecture view).
> ~~**13**~~ **DONE 2026-07-17** — the product decision that blocked it is made (server is
> the end goal, SSO), so it shipped as a server-ready identity model. See its entry below.
> ~~**23**~~ **DONE 2026-07-15** (ABI fixed AND the item-21 migration regression tests written).
> ~~**29**~~ **DONE 2026-07-16** (architecture top-bar consolidation — Phase 1 + Phase 2 +
> Phase 3 B2/B3 `2ec92dc`, B1 colour-by-type + Preferences `a954938..6fab0d5`).
> ~~**31**~~ **DONE 2026-07-16** (keyboard-accessible section re-parenting — ⇄ "Move to…"
> select on heading rows, reusing item 28's `headings:reparent` IPC + `canReparent` guard).
...
13. ~~Last-modified attribution (user + timestamp) on requirements~~ — **DONE 2026-07-17**. The blocker was never work, it was the product decision the old entry named; the user settled it: **the end goal is a server** (team collaboration + central org system-of-record, SSO/OAuth; source of truth server-vs-file deliberately still open). That makes identity a *key*, not a label, so this shipped as a server-ready model rather than a name string. Plan: `~/.claude/plans/compressed-chasing-wave.md`. Delivered: `users` table per project file (roster) keyed by a **uuid** — the integer PK is per-file and would collide the moment a server ingests two projects; `created_by`/`updated_by` nullable FKs on `requirements`; `identity.json` in `userData` for who you are across projects (`src/main/identity.ts`); "You" + "People" sections in the Settings modal; last-modified line in the drawer and author in the Dashboard activity feed.
    **Three decisions worth not re-litigating:** (a) **the renderer never asserts its author** — handlers stamp from main-process identity, ignoring any `updated_by` in input, because renderer→main is the same trust boundary client→server will be (there is a mutation-verified test for exactly this); (b) **legacy rows stay NULL and are never backfilled** — NULL means "predates attribution", and stamping them would fabricate a record of who edited what, the wrong failure for a system-of-record; (c) **you are enrolled in a file's roster lazily, on first write** — opening someone's project must not claim you touched it.
    **Deliberately NOT built: passwords or a login screen.** Against a local SQLite file they are not a security boundary — anyone with the `.reqarch` reads every row regardless — and the credential code would be discarded when SSO lands. When it does: `external_id` takes the OAuth subject, `email` reconciles the roster against the org directory, and `currentUserRowId` is replaced by the session's user; the stamping call sites do not change. Scope is requirements only; elements/connections/modules/headings carry `updated_at` already and extending attribution to them is mechanical.
...
23. **Migration regression tests for the item-21 folder split** — raised by item 21's final whole-branch review (opus, 2026-07-14) as its only new finding. `src/main/db/migrations.test.ts` already holds 5 migration tests, so the home exists. The item-21 design spec's Testing section mandated three: the **split** case (parent with both requirements and children → folder + same-name module, prefix/counter preserved, requirements repointed), the **flip-only** case (parent owning neither requirements nor headings → flipped to folder, no split), and a **second run proving idempotence**. The plan overrode that (plan line 243, "vitest cannot open the DB") and substituted live-verify. The trade was legitimate, but it left real gaps: live-verify exercised exactly **one** shape — a 2-level tree taking the split branch. The **flip-only branch** and **3+ level nesting** have two independent hand-traces (implementer + reviewer) but **no empirical evidence and no automated coverage**.
    **✅ DONE 2026-07-15** (`src/main/db/folderSplit.test.ts`, 6 tests). The ABI prerequisite was fixed first (see below), then the tests were written. All three mandated cases are covered — **split** (folder keeps children, new same-name module inherits prefix/counter, requirements + headings repointed, `req_id`s byte-identical), **flip-only** (parent owning nothing flips with no module invented), **idempotence** (three consecutive runs, table-identical) — plus the two gaps the item-21 review named: **3-level nesting** (both upper levels convert, leaf untouched) and the **soft-deleted-child** branch (`c.deleted_at IS NULL` — a parent whose only child is deleted keeps minting IDs), and soft-deleted requirements following their module. **Proven load-bearing by mutation testing:** five separate mutations of `migrations.ts` (drop counter inheritance, `n > 0`→`n >= 0`, never flip to folder, drop the deleted-child guard, leave soft-deleted requirements behind) each failed the tests that should catch them.
    **How it was fixed, since the old framing here was half-right and stopped one step short.** The diagnosis was correct: `postinstall` runs `electron-rebuild -f -w better-sqlite3`, building for Electron's ABI 125 — that is *why the app runs* — while the test runner's node is ABI 127, and one build cannot serve both runtimes. What it missed is that nothing requires both runtimes to share *one copy of the package*. `package.json` gained `"better-sqlite3-node": "npm:better-sqlite3@^12.11.1"` (an npm alias: same package, same version, second directory) and `vitest.config.ts` aliases `better-sqlite3` → `better-sqlite3-node`. `electron-rebuild -w better-sqlite3` only ever targets the `better-sqlite3` **directory**, so the alias copy keeps its node-ABI prebuild. App gets ABI 125; tests get ABI 127. **30 lines across 3 files**, verified to survive a clean `npm ci`, with the app confirmed still opening its DB. Parity guard: `src/main/db/sqliteAbi.test.ts`.
    **⚠️ Probing trap, recorded so nobody repeats it:** `require('better-sqlite3')` does NOT load the native binary — it only loads JS, and the addon is dlopen'd lazily inside the `Database` constructor. `node -e "require('better-sqlite3')"` therefore succeeds regardless of ABI and proves **nothing**. Probe with `new Database(':memory:')`.
    **The standing "no migration/handler tests" rule is repealed.** Items 26, 27 and 29-Phase-2 each shipped a DB column with zero automated coverage citing this item; that justification no longer exists.
...
29. **Architecture top-bar consolidation.** Gather all canvas tools into the top bar beside `+ Object` — the user's stated direction is "all features for the architecture section in the top bar" — and add font and colour controls. **Design spec: `docs/superpowers/specs/2026-07-15-architecture-top-bar-design.md`** (top bar) and `docs/superpowers/specs/2026-07-15-architecture-fill-colour-design.md` (fill colour). **Phase 1 DONE** 2026-07-15 (`72f95ad`) — split selection model + `Style ▾`/`Type ▾` popovers in the top bar, styling deleted from `ElementPanel`/`ConnectionPanel`. **Phase 2 DONE** 2026-07-15 (`c5a0654..7216d94`) — per-object `fill_color` column, paired border/fill swatch palette, fill rendering (wins over the container drop-zone overlay), `Fill` row in `Style ▾`; live-verified in the built app (all 7 checks passed, see `handoff.md`'s "Item 29 Phase 2" section for the DB evidence). **Phase 3 DONE** — B2 (snap) + B3 (duplicate) 2026-07-15 (`2ec92dc`); B1 (optional colour-by-type) 2026-07-16 (`a954938..6fab0d5`): a NEW Settings modal (⚙ in the navy header) housing an extensible Preferences section + a type-colour editor, border-only, toggle persisted app-global in localStorage `reqarch.prefs.colourByType`. Design: `docs/superpowers/specs/2026-07-16-preferences-and-type-colours-design.md`. **Item 29 is CLOSED.**
