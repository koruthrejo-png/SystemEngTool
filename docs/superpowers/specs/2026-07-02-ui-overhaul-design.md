...
> **Status as of 2026-07-15.** **Done: 1-10, 15-22, 24-27.**
...
> blocked on the main-process test suite being runnable at all), **28** (drag whole sections),
> **29** (architecture top-bar consolidation).
...
28. **Drag whole sections.** Make heading rows draggable so a section moves with its subheadings and requirements, for easy reorganisation. Today only requirement rows are draggable, and that drag only ever *re-parents* (`updateRequirement(id, { headingId })`) — it never reorders — **NOT STARTED** (verified 2026-07-15: heading rows have no `draggable` attribute and are drop targets only; there is no `headings:reparent` IPC channel; `updateHeading` (`headings.ts:41-48`) writes `title` only; `moveHeading` (`:51-67`) swaps sibling `position` scoped by `parent_id IS ?`, so it cannot cross levels. Missing at **every** layer — UI, IPC, and DB — which is why it was split out rather than bundled with item 24. **Descendants need no explicit move:** parentage is stored as `parent_id`/`heading_id` FKs, so children follow their parent automatically and `buildOutline` re-nests them on the next render — cf. `deleteHeading`, which *must* touch descendants precisely because it removes the parent they point at.)
29. **Architecture top-bar consolidation.** Gather all canvas tools into the top bar beside `+ Object` — the user's stated direction is "all features for the architecture section in the top bar" — and add font and colour controls. Item 25 moved Layers there as the first step — **NOT STARTED** (verified 2026-07-15: object colour today is a bare `<input type="color">` in `ElementPanel`, not a top-bar control; there is no font control anywhere; the `layers` table has no colour column — the panel's `● ◐ ○` dots are visibility-state glyphs, not layer colours.)
