...
Backlog in `docs/superpowers/specs/2026-07-02-ui-overhaul-design.md` §6: **18** DONE, **19** DONE, **20** DONE (toolbar rework), **21** rework requirements file structure — pure container files/folders that modules live in, replacing module-in-module nesting (**USE PLAN MODE** — user reminder), **22** connection line editing — easy delete + line style (dotted/dashed) + arrow/marker types (**USE PLAN MODE** — user reminder).

### Connection Line Editing (backlog item 22) — SPEC + PLAN WRITTEN, execution NOT started
Session 2026-07-14 brainstormed → spec → plan for item 22. Spec `docs/superpowers/specs/2026-07-14-connection-line-editing-design.md` (`9d75716`). Plan `docs/superpowers/plans/2026-07-14-connection-line-editing.md` (`63c7c88`, 7 tasks, ready for subagent-driven-development). Decisions locked with the user: styling lives **per connection** (not per-type); **Delete/Backspace** removes a selected connection (connections only — never a block; undoable via existing `removeConnection`); line style **solid/dashed/dotted**; **independent start + end arrow markers** (None/Open/Filled each); a **newly drawn connection defaults to a single filled arrow at the target** (`marker_end='arrowclosed'`, `marker_start='none'`); **legacy connections stay bare** (NULL columns → solid/none/none render fallback, no retroactive arrows). Additive backend: 3 nullable columns (`line_style`, `marker_start`, `marker_end`) on `architecture_connections`; style edits ride the existing `updateConnection` undo capture via `CONNECTION_PROP_KEYS`; two pure render helpers `dashArray`/`edgeMarker` in a new `ArchitectureCanvas/edgeStyle.ts`. Plan tasks: 1 backend, 2 store undo-keys, 3 edgeStyle helpers, 4 edge rendering (index.tsx + EdgeLabel), 5 ConnectionPanel selects, 6 Delete-key predicate+wire, 7 verify+live-verify. **NEXT: execute this plan via subagent-driven-development** (base commit `63c7c88`; append a new `.superpowers/sdd/progress.md` ledger section). NOTE for the executor: the store's `undo()`/`redo()` run `loadArchitecture()` in a `finally` (touches many `window.api.*`), so focused store tests must invoke the captured `undoStack[0].undo()` command directly rather than the store `undo()` action — the plan's Task 2 test already does this.

## Next Step: PLANNING + REFINING (user directive)
User wants focus on **planning and refining what we have currently**, not net-new build. Open: item **21** (requirements file-structure rework) still flagged for **plan mode** (brainstorm → spec → plan). Item **22** (connection line editing) is now SPEC+PLAN complete — execute it next (see the section directly above). The **Layers** plan is EXECUTED and shipped (see the Layers COMPLETE section above). Other follow-ups: codebase-wide promise error-surfacing pass; batched `aria-pressed`/`role`-and-keyboard a11y pass (ArchitectureNav/InterfaceNav/LayerPanel rows are `<div onClick>`); heading-anchor on search section clicks. New-feature flow stays: brainstorm spec → superpowers:writing-plans → subagent-driven-development (append new ledger section).
