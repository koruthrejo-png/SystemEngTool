...
### Architecture Left-Nav + Per-Architecture Interfaces + Object-Name Columns — EXECUTING (subagent-driven)
Spec: `docs/superpowers/specs/2026-07-12-architecture-nav-and-per-architecture-interfaces-design.md` (commit `a08ea10`). Plan: `docs/superpowers/plans/2026-07-12-architecture-nav-and-per-architecture-interfaces.md` (commit `8077fd6`). Fifteenth ledger section in `.superpowers/sdd/progress.md` (base `8077fd6`). Pure renderer — no DB/handler/preload/type changes. 5 tasks:
- **Task 1 — DONE** (`6b858fe`): store `interfaceArchFilter: number | 'all'` (session-only, default `'all'`, reset in `loadProject`) + `setInterfaceArchFilter`. Review clean.
- **Task 2 — in progress**: new `ArchitectureCanvas/ArchitectureNav.tsx` left sidebar (list + switch/create/rename/delete, `×` hidden when 1) REPLACES the top `ArchitectureTabs` strip (strip + its test DELETED); App.tsx architecture panel becomes `[ArchitectureNav w-56 | canvas | properties]`.
- **Task 3 — pending**: shelve Component Library (unmount in `ArchitectureCanvas/index.tsx`, file kept) + add a type-picker `Select` next to the toolbar `+ Object` so typed-block creation survives.
- **Task 4 — pending**: `InterfaceRegister/rows.ts` gains `fromName`/`toName`/`architectureId` on `InterfaceRow`; register renders mandatory **From Name** + **To Name** columns, order `Interface ID | From | From Name | To | To Name | …optional`.
- **Task 5 — pending**: new `InterfaceRegister/InterfaceNav.tsx` left sidebar ("All architectures" + per-architecture entries) sets `interfaceArchFilter`; register filters rows client-side + scopes `+ New Interface` pickers/stamp to the selected architecture; App.tsx interfaces panel gains the sidebar. Ends with build + Playwright live-verify.
Design decisions locked (from brainstorm): typed blocks via `+ Object` type dropdown (not palette); top strip removed entirely; Interfaces nav keeps an "All" entry; object names as SEPARATE From Name/To Name mandatory columns; interface filter client-side + session-only. NOTE: an image was referenced in the request but never arrived — design built from the text description.
...
## Branch
`main` — all work committed directly to main. Latest commit at handoff: `c56dd84` (docs: ledger) — end of the Multiple Architectures sub-project (`60700a2..cbe4b4e`; final-review fixes `cbe4b4e`).
