...
### Architecture Tab: Multiple Architectures + Layers — SPEC + PLAN WRITTEN (not yet executed)
Combined spec: `docs/superpowers/specs/2026-07-11-multiple-architectures-and-layers-design.md` (commit `a5016b1`). Two sequential sub-projects (layers depend on the architecture entity, so multiple-architectures ships first):
- **Sub-project 1 — Multiple Architectures** (plan `docs/superpowers/plans/2026-07-11-multiple-architectures.md`, commit `f9b6f82`): new `architectures` table; `architecture_elements`/`architecture_connections` gain nullable `architecture_id`; idempotent migration creates a per-project **"Default"** architecture and backfills. Fully independent diagrams (each its own blocks). `elements/connections.list` gains an OPTIONAL `architectureId` filter (non-breaking — the project-wide Interface Register keeps loading all connections; the canvas loads one architecture). Store: `architectures` + `activeArchitectureId` (persisted per project in localStorage `reqarch.activeArchitecture.<projectId>`), `loadArchitectures`/`setActiveArchitecture`/`add`/`rename`/`removeArchitecture`. UI: sub-tab strip `ArchitectureTabs.tsx` above the canvas (create/rename/delete, last-one delete guard). Interface Register gains a toggleable Architecture column. 6 tasks, ready for subagent-driven execution.
- **Sub-project 2 — Layers** (requirements captured in the spec, NOT yet planned): per-architecture visibility layers; many-to-many object↔layer; 3-state per layer (Visible → Faded → Hidden); object shows normally if any layer Visible, else faded if any Faded, else hidden; no-layer objects always visible; connectors get own layer assignment + endpoint-wins auto-hide. Brainstorm into its own plan AFTER sub-project 1 ships.

## Next Step: pick the next slice

Nothing executing. Options: (1) execute the **Multiple Architectures** plan (`docs/superpowers/plans/2026-07-11-multiple-architectures.md`) via subagent-driven-development; then (2) brainstorm the **Layers** sub-project into a plan. Separately, next major *pillar* per build order is **V&V** (requirements → architecture → interfaces → **V&V**). Ticketed follow-ups still open: promise error-surfacing pass, batched aria-pressed a11y pass, search heading-anchor. Ticketed follow-ups: codebase-wide promise error-surfacing pass; batched `aria-pressed` toggle a11y pass; heading-anchor on search section clicks. Follow the same flow: brainstorm spec → superpowers:writing-plans → subagent-driven-development (append new ledger section).
