...
## ITEM 23 (better-sqlite3 ABI) — FIXED 2026-07-15. Suite now 53 files / 375 tests, ZERO failures.
  ...
  - FOLLOW-UP DONE same session: `src/main/db/folderSplit.test.ts` (6 tests) — the item-21 folder-split migration regression tests that item 23 had blocked. Covers the 3 mandated cases (split / flip-only / idempotence) + the 2 gaps the item-21 review named (3-level nesting, soft-deleted-child branch) + soft-deleted requirements following their module. **Mutation-tested:** 5 mutations of migrations.ts (drop counter inheritance; n>0 -> n>=0; never flip to folder; drop the c.deleted_at guard; leave soft-deleted requirements behind) each failed the right tests. Suite: 54 files / 381 tests, zero failures. Item 23 fully closed.

## Item 29 Phase 3 — B2 (snap) + B3 (duplicate) — DONE 2026-07-15 (`2ec92dc`). B1 NOT done (needs own brainstorm).
  - **Spec error found:** it claimed B3 "reuses addElement". CreateElementInput had NO style fields and createElement silently ignored them — the exact gap item 22 recorded on connections ("a caller has never needed one"). B3 IS that caller. Extended CreateElementInput + the INSERT. Load-bearing: one create call = ONE undo entry; create-then-style would push two, so the first Cmd+Z would strip styling and leave a naked duplicate.
  - User decisions: Cmd+D only (no bar button); duplicate copies size + container as well as style.
  - First feature this session with real backend coverage (item 23 fixed earlier today) — elements.test.ts covers the style path AND that `+ Object` still yields a plain 160x80 block.
...
- Task 8: complete — build clean, full suite 409/409, tsc clean; final opus review (no Critical; 1 Important + 2 Minor) fixed in 6fab0d5; live-verified on thermal.
- Final review Important FIXED (6fab0d5): backfill was per-row (is_built_in AND color IS NULL AND name) → reverted a user who cleared a BUILT-IN's colour to None (NULL is ambiguous for built-ins: legacy-unseeded vs user-cleared). Now per-PROJECT: only backfill projects where EVERY built-in is still NULL (HAVING COUNT(color)=0). Real legacy data is all-NULL → still fully backfilled; mixed states (only reachable via user edits) skipped. Middle regression test confirmed fail-then-pass. Known residual: clearing ALL built-ins to NULL re-qualifies the project → re-seeds next launch (extreme, acceptable).
- Minor fixed: elementTypes now loaded in loadProject (Settings Type Colours populated from any tab). Minor noted: native colour picker on Border/Fill still spams undo per drag (pre-existing Phase 1/2, not this feature).
- Live-verify (standalone Playwright script, thermal): backfill applied on real DB (5 built-ins show exact palette colours in Settings); toggle ON persists localStorage='true'; types load from Requirements tab. Caveat: thermal active arch has only a System-typed object (type colour == NAVY fallback), so on/off is visually identical on it — the distinct resolution is unit-proven in nodes.test.ts. thermal left with toggle OFF; built-in colours now permanently backfilled (intended, like prior migrations).
- **Item 29 B1 COMPLETE → item 29 fully closed.** Commits a954938..6fab0d5.

## Items 32/33 phase 1 — CSV/ReqIF export + CSV import (subagent-driven, 2026-07-22)
Plan: `docs/superpowers/plans/2026-07-22-requirements-csv-reqif-export.md`. Base `26889f9`. Order 32 first of 32->34->35.
- Task 1: complete (2991415, review clean — sonnet) — src/main/export/model.ts: ExportRow/ParsedRow, escapeXml, buildSectionPath (cycle-guarded), findHeadingByPath. 5/5 tests, both typechecks clean.
  - Minor (non-blocking, for final review): buildSectionPath cycle guard has no direct unit test (unreachable today via FK-constrained headings).
- Task 2: complete (d66b6a1, review clean — sonnet) — src/main/export/csv.ts: rowsToCsv + parseCsv (RFC 4180 state machine) + CORE_COLUMNS. 5/5 tests incl round-trip, typechecks clean.
  - Minors (non-blocking): round-trip test doesn't compose a comma/quote field through both fns (escaping verified in isolation only); `module` column is export-only — ParsedRow has no `module` field, so import ignores it (by design: import targets one moduleId).
- Task 3: complete (840b9f9, review clean — sonnet) — src/main/export/reqif.ts: rowsToReqif (write-only). 5/5 tests, typechecks clean. Escaping audited: every user value routes through escapeXml; enum idx via imported union arrays.
  - Minors (non-blocking): objId() char-strip could collide adversarial reqIds (write-only, own data); `module` omitted from ReqIF output (brief STRING_ATTRS decision).
- Task 4: complete (7dac37d, review clean — sonnet) — src/main/export/merge.ts: planImport + resolveDerivedFrom. 5/5 tests, typechecks clean. No issues. Enum check runs before update/create split (matched row w/ bad enum is skipped — correct per spec).
- Task 5: complete (1694d63 + fix 613197e, review opus). io.ts handler + types (ExportResult/ImportResult) + main/index register + preload + api.d.ts. Both typechecks clean; suite 464/465 (1 PRE-EXISTING App.test.tsx failure, unrelated — fails identically on base 7dac37d, predates this session).
  - Important FIXED (613197e): forward-ref derivation links to CREATED rows were lost — file reqId mapped only for updates, but derived_from references file reqIds. Now mapped for creates too. Reviewer-prescribed one-liner, verified.
  - Notes (non-blocking): single-module export drops cross-module derived_from links (correct for module-scoped file); create-path 2nd updateRequirement not in try/catch (inputs pre-validated by planImport).
  - **⚠️ PRE-EXISTING App.test.tsx failure predates this feature — flag in handoff; not ours to fix here but should be tracked.**
- Task 6: complete (a8b8c7a, review clean — sonnet) — store actions exportCsv/exportReqif/importCsv via run(). io.test.ts 3/3, store suite 59/59, typechecks clean.
  - Minors (non-blocking, brief-inherited): no null-project export test (guard present + correct); cancelled-dialog test asserts only list-not-called.
- Task 7: complete (08f9611, review clean — sonnet) — Export ▾ menu (4 items) + Import CSV button in RequirementsList toolbar, HeaderMenu reused. RequirementsList suite 67/67, typecheck clean. No findings.
- Post-task gap FIXED (2cbcd20): entry_type column (added 2026-07-21, user-visible) was absent from export/import — CSV round-trip silently dropped it. Now round-trips across model/csv/reqif/io + tests. Surfaced by the (interrupted) final review's entry_type lead. Export suite 20/20.
- Gate: typechecks clean (node+web), electron-vite build clean (3 targets), full suite 467/468 (1 PRE-EXISTING App.test.tsx failure — expects an "Open" button moved into project switcher on 2026-07-21c, test never updated; predates feature).
- Final review (opus, rerun after 1st was interrupted): READY TO MERGE, zero Critical/Important. Minor 1 (silent section loss on fresh-module import) FIXED 785a3d2 (now reported in errors). Minors 2/3 (CSV formula injection — deliberate fidelity tradeoff; XML control chars) recorded as follow-ups.
- Docs updated: handoff.md session section + spec §6 items 32/33 annotated phase-1 done.
- **NOT live-verified: native OS dialogs block the Playwright driver. Pure pipeline unit-tested + handler opus-reviewed; needs a user hands-on pass (app launch) to close fully.**
- **⚠️ PRE-EXISTING stale test: src/renderer/src/App.test.tsx expects an "Open" button moved to project switcher 2026-07-21c — fails on base, not this feature. Someone update it.**
- ITEM 32 phase 1 COMPLETE (pending user hands-on). Next per user order: item 34 (change history), then 35 (baselines).

## Feature: AC free-text + Verification status (2026-07-23) — base 71054e1
Plan: docs/superpowers/plans/2026-07-23-ac-freetext-verification-status.md
Spec: docs/superpowers/specs/2026-07-23-ac-freetext-verification-status-design.md
Replaces item-7 structured AC checklist with free-text AC + one verification_status per requirement; fixes item-32 AC round-trip.
- Task 1: complete (c403673, review clean — sonnet). Enum VERIFICATION_STATUSES + Requirement.verificationStatus + UpdateRequirementInput field; migration adds verification_status column; requirements handler rowToRequirement/updateRequirement map it; In Progress chip (amber). Additive only, no removals. typecheck+build clean; renderer suite 365/366 (1 pre-existing App.test.tsx fail). Backend handler test authored (dark ERR_DLOPEN). Note: 5 renderer test fixtures got verificationStatus added (non-optional field) — verified minimal by reviewer.
- Task 2: complete (ed8b069+b671762, review clean — sonnet). Renderer swap: drawer AC checklist → free-text Textarea + Verification Select; table ac cell → EditableCell(acceptanceCriteria); store dropped acItems/acSummary + 5 AC actions + refreshAc + acSummary.ts; filter gained verificationStatus enum attr. Backend AC left intact (Task 3 removes). typecheck clean, renderer 351/352 (pre-existing App.test.tsx). Minor: 2 commits (git add mishap, cosmetic). Reviewer grepped zero dangling AC refs.
- Task 3: complete (a053c13+967ded0, review clean — sonnet). Removed AC child backend: acceptanceCriteria.ts handler + IPC reg + preload bridge + api.d.ts block + AC_STATUSES/AcStatus/AcceptanceCriterion/UpdateAcceptanceCriterionInput types. Migration: deleted child-table DDL + item-7 split; added collapse-and-drop (guarded on sqlite_master, join by position,id newline, DROP TABLE, idempotent). requirements.acceptance_criteria column preserved. Migration test authored (dark). typecheck+build clean, renderer 351/352, grep zero dangling AC-type refs. Minors: 2 commits (cosmetic); getItems orders position,id (improvement over brief).
- Task 4: complete (81d4876, review clean — sonnet). Export/import round-trip: verification_status added to CORE_COLUMNS (after priority), model ExportRow/ParsedRow, reqif ENUMS (auto datatype+attrdef+value), merge ENUM_SETS (label verification_status), io.ts assembleRows+toUpdateInput. acceptance_criteria round-trips (was the item-32 bug). export vitest 22/22, typecheck clean. TDD RED-first confirmed. Minors: io.ts wiring untested at that layer (matches status/priority convention); parseCsv order-independent by name.
- Task 5: complete (docs commit pending). LIVE-VERIFY ALL PASS via driver + main-process dialog stub:
  - Gate: typecheck+build clean, vitest 373/374 (1 pre-existing App.test.tsx "open" button).
  - Migration on real DBs: Satellite Demo (SAT-001 2 items→"...deployed\ntestv2", SAT-016→"battery level check ui", verification Unverified, child table DROPPED, 17 reqs/reqIds intact); SmokeTest (SRS-0002→"Response time under 200ms", table dropped). Idempotent on relaunch (table stays gone, AC unchanged, no re-collapse).
  - Drawer: VERIFICATION select (Unverified/In Progress/Passed/Failed) below Type/Status/Priority; AC now plain Textarea (no checklist). Set DEM-0002 verification=Passed via drawer → persisted to DB.
  - Item-32 round-trip (THE BUG FIX): export CSV header has acceptance_criteria(col5)+verification_status(col12); DEM-0002 exported verification=Passed. Import: DEM-0002 updated (AC "Locks after 5 tries\nUnlocks after 15 min" landed in REAL column, now UI-visible; verification Failed), DEM-0009 created (blank req_id→minted, AC "Single AC line", In Progress), bad "Nope" verification row SKIPPED. Banner "Imported 1 new, 1 updated, 1 skipped".
  - Scratch pollution (harmless): SmokeTest got DEM-0009 + DEM-0002 AC/verification edits (test data on a scratch project); Satellite Demo permanently migrated (shipped behavior, no test rows added).
- FEATURE COMPLETE pending final whole-branch review. Commits c403673 (T1), ed8b069+b671762 (T2), a053c13+967ded0 (T3), 81d4876 (T4).
- FINAL WHOLE-BRANCH REVIEW (opus, 71054e1..c8467fd): READY TO MERGE — zero Critical, zero Important. Verified: migration idempotent/transactional/correctly-ordered (no hazard vs item-21 folder-split — neither touches requirement id), item-7 split fully removed, create-DDL keeps acceptance_criteria TEXT, verification_status column added before collapse; handler mapping + blank-leaves-existing update semantics + create-path default; CSV writer/CORE_COLUMNS positional agreement + name-keyed parser, ReqIF datatype+attrdef+value all coherent, merge ENUM_SETS validates verification, AC round-trips both ways incl. create path; renderer removal complete, zero dangling refs. Carried Minors (non-blocking): drawer save() can't clear AC to empty (ac || undefined; mirrors Source/Rationale; table cell CAN clear) — revisit only if drawer-clear wanted; pre-existing App.test.tsx red; main-process tests dark.
- FEATURE COMPLETE + READY TO MERGE (on main). Commits 71054e1(plan)..c8467fd + final docs.
